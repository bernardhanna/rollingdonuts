                    ...
                    @foreach ($selectedFaqs as $index => $faq)
                        <div class="border-b-2 border-black-full lg:bg-white">
                            <div id="accordion-open-heading-{{ $index }}">
                                <button type="button" class="flex items-center justify-between w-full font-medium text-left py-3" data-accordion-target="#accordion-open-body-{{ $index }}" aria-expanded="false" aria-controls="accordion-open-body-{{ $index }}" x-data="{ isRotated: false }" x-on:click="isRotated = !isRotated">
