        ...
        ?>
            <?php foreach ($methods as $method) : ?>
                <tr class="payment-method<?php echo !empty($method['is_default']) ? ' default-payment-method' : ''; ?>">
                    <?php foreach (wc_get_account_payment_methods_columns() as $column_id => $column_name) : ?>
                        <td class="flex flex-col woocommerce-PaymentMethod woocommerce-PaymentMethod--<?php echo esc_attr($column_id); ?> payment-method-<?php echo esc_attr($column_id); ?>" data-title="<?php echo esc_attr($column_name); ?>">
                            <?php
...
<?php if (WC()->payment_gateways->get_available_payment_gateways()) : ?>
    <a class="button text-center text-yellow-primary hover:text-black-full text-mob-lg-font lg:text-mob-md-font font-medium py-3 bg-black-full hover:bg-primary-yellow rounded-lg-x w-full lg:w-1/2 hover:bg-yellow-primary mb-4 border-solid border-2 border-yellow-primary hover:border-black-full" href="<?php echo esc_url(wc_get_endpoint_url('add-payment-method')); ?>"><?php esc_html_e('Add payment method', 'woocommerce'); ?></a>
<?php endif; ?>