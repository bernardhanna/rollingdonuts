...
?>
<div class="mx-auto px-4 pb-24 lg:max-w-max-1549">
    <?php if ('yes' === get_option('woocommerce_enable_myaccount_registration')) : ?>
        <div class="border-black-full border-2 border-solid rounded-md-32 p-12" x-data="{ activeTab: 'sign-in', showLostPassword: false }">
            <div class="text-sm font-medium text-center">
                <ul class="flex flex-wrap -mb-px text-md-font font-reg420">
                    <li class="w-1/2 pb-4">
                    <a href="#"
                        @click="activeTab = 'sign-in'; showLostPassword = false"
                        :class="{ 'active dark:text-blue-500': activeTab === 'sign-in', 'text-gray-500': activeTab !== 'sign-in' }"
                        x-text="showLostPassword ? 'Forgot Password' : 'Sign In'">
                        </a>
                    </li>
                    <li class="w-1/2">
                    <a href="#" @click="activeTab = 'register'; showLostPassword = false" :class="{ 'text-blue-600': activeTab === 'register', 'text-gray-500': activeTab !== 'register' }">Register</a>
                    </li>
                </ul>
            </div>
        ...
        <div x-show="activeTab === 'register'">
            ...
            text-mob-xs-font font-laca font-light pl-11 flex w-full" placeholder="<?php esc_attr_e('First name*', 'woocommerce'); ?>" name="first_name" id="reg_first_name" autocomplete="given-name" />
                </div>
                <div class="w-1/2">
                    <label for="ml-2 text-mob-xs-font font-reg420 reg_last_name"><?php esc_html_e('Last Name', 'woocommerce'); ?>&nbsp;<span class="required">*</span></label>
                    <input type="text" required class="woocommerce-Input woocommerce-Input--text input-text rounded-lg-x h-input text-black-secondary
            text-mob-xs-font font-laca font-light pl-11 flex w-full" placeholder="<?php esc_attr_e('Surname*', 'woocommerce'); ?>" name="last_name" id="reg_last_name" autocomplete="family-name" />
                </div>
            </div>

            <!-- Email Input -->
            <div class="mb-4">
                <label for="reg_email ml-2 text-mob-xs-font font-reg420"><?php esc_html_e('Email Address', 'woocommerce'); ?>&nbsp;<span class="required">*</span></label>
            ...
            text-mob-xs-font font-laca font-light pl-11 flex w-full" placeholder="<?php esc_attr_e('Email*', 'woocommerce'); ?>" name="email" id="reg_email" autocomplete="email" />
            </div>
            ...
            text-mob-xs-font font-laca font-light pl-11 flex w-full" placeholder="<?php esc_attr_e('Password*', 'woocommerce'); ?>" name="password" id="reg_password" autocomplete="new-password" />
            </div>

            <!-- Confirm Password Input -->
            <div class="mb-4">
                <label for="confirm_password ml-2 text-mob-xs-font font-reg420"><?php esc_html_e('Confirm Password', 'woocommerce'); ?>&nbsp;<span class="required">*</span></label>
                <input type="password" required class="woocommerce-Input woocommerce-Input--text input-tex rounded-lg-x h-input text-black-secondary
            text-mob-xs-font font-laca font-light pl-11 flex w-full" placeholder="<?php esc_attr_e('Confirm Password*', 'woocommerce'); ?>" name="confirm_password" id="confirm_password" />
            </div>

            <!-- Checkbox for data handling -->
            <div class="mb-4">
                <label class="woocommerce-form__label woocommerce-form__label-for-checkbox woocommerce-form-login__rememberme">
                    <input required class="woocommerce-form__input woocommerce-form__input-checkbox bg-white mr-2 h-[20px] w-[20px] rounded-sm" type="checkbox" name="privacy_agreement" id="privacy_agreement" />
                    <span class="text-sm-font font-regular"><?php esc_html_e('I agree with the handling of my data in accordance with the company privacy policy.', 'woocommerce'); ?></span>
                </label>
            </div>
