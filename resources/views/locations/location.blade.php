...
<section class="relative max-mobile:-top-20" x-data="{ windowWidth: window.innerWidth }"
         ...
         class="p-0 pt-4 mx-auto tablet-sm:max-w-max-1341 mobile:px-4">
    @php
    $args = array(
...
$fields = get_fields();
        ...
        <div class="relative flex flex-col pt-10 mb-4 bg-white location-item mobile:pt-0 mobile:border-black mobile:border-4 mobile:border-solid mobile:rounded-md-32 mobile:flex-row tablet-sm:pt-6 tablet-sm:pb-3">
            <div class="flex flex-col mobile:p-4 lg:p-0 tablet-sm:mr-auto tablet-sm:ml-auto lg:w-30">
                <h4 class="hidden w-full pt-3 pb-8 leading-normal max-mobile:block text-font-28 tablet-sm:text-md-font font-reg420 text-black-full">{{ get_the_title() }}</h4>
            ...
            <div class="relative flex flex-col w-full pr-4 mobile:w-3/5 lg:w-2/3 lg:-top-4 lg:right-2">
                <div class="flex flex-col pt-10 mobile:pt-0">
                    <h4 class="w-full pt-3 leading-normal max-mobile:order-1 max-mobile:hidden text-font-28 tablet-sm:text-md-font font-reg420 text-black-full">{{ get_the_title() }}</h4> <!-- Title -->
                    @php
...
<div class="flex flex-col w-full pt-0 max-mobile:order-2 tablet-sm:pt-5 max-mobile:flex-wrap tablet-sm:justify-between notebook:flex-row tablet-sm:flex-row">
                        <span class="flex items-start font-medium tablet-sm:items-center font-laca text-reg-font tablet-sm:text-base-font laptop:text-sm-md-font text-black-primary"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            ...
                            <a href="tel:{{ $fields['phone_number'] }}" class="flex items-center font-medium font-laca text-reg-font tablet-sm:text-base-font laptop:text-sm-md-font text-black-primary"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <div x-data="{ openPanel: null } bg-white" class="max-mobile:order-4 border-2 border-solid border-black rounded-sm-12 boxshadow mt-[10px] pt-2 pb-2 pl-0 mobile:pr-4">
                <div class="flex flex-col" id="accordion-collapse-{{ $post_id }}" x-data="{ isOpen: window.innerWidth >= 1024 }" x-init="window.addEventListener('resize', () => { isOpen = window.innerWidth >= 993 })">
                    ...
                    <button @click="isOpen = !isOpen" type="button" class="flex items-center justify-between w-full pl-5 pr-5 font-medium text-left text-gray-500 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-800 dark:border-gray-700 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800 tablet-sm:py-4">
                        <span class="relative text-black-primary text-sm-md-font font-reg420 ">Opening Hours</span>
                        <!-- SVG when open -->
...
<a class="max-tablet-sm:mb-4 w-full tablet-sm:w-[340px] h-[48px] tablet-sm:h-[56px] flex justify-center rounded-btn-72 bg-yellow-primary hover:bg-white border-black border-solid border-3 items-center" href="{{ $fields['order_for_collection_link']['url'] }}" target="{{ $fields['order_for_collection_link']['target'] }}">
                        <span class="font-medium text-black-full text-base-font">Order for Collection</span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="33" height="32" viewBox="0 0 33 32" fill="none">
