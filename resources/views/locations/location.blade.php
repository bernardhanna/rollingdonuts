...
@if($query->have_posts())
    @while($query->have_posts()) @php $query->the_post(); @endphp
        ...
        @endphp
        <div class="location-item mb-4 relative lg:border-black lg:border-4 lg:border-solid lg:rounded-md-32 flex flex-col lg:flex-row p-4 bg-white">
            <div class="flex flex-col w-full lg:w-1/3 lg:mr-4">
                    @if(has_post_thumbnail())
                        ...
                        <img class="object-cover w-full h-[270px] lg:h-full" src="{{ $thumbnail_url[0] }}" alt="{{ get_the_title() }}"> <!-- Featured Image -->
                    @endif
                </div>
            <div class=" w-full lg:w-2/3 flex flex-col justify-between">
                <div class="flex flex-col">
                    <h4 class="text-md-font font-reg420 text-black-full w-full">{{ get_the_title() }}</h4> <!-- Title -->
                    @php
                        $fields = get_fields(); // This will return all ACF fields for the post
                    @endphp
                    <div class="flex lg:justify-between lg:flex-row w-full">
                        <span class="flex items-center font-laca lg:text-reg-font text-sm-md-font text-black-primary font-medium"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
                            ...
                            <a href="tel:{{ $fields['phone_number'] }}" class="flex items-center font-laca lg:text-reg-font text-sm-md-font text-black-primary font-medium"><svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
            ...
            <div x-data="{ openPanel: null } bg-white" class="border-2 border-solid border-black rounded-sm-12 boxshadow">
                <div class="flex flex-col" id="accordion-collapse-{{ $post_id }}" x-data="{ isOpen: window.innerWidth >= 1024 }" x-init="window.addEventListener('resize', () => { isOpen = window.innerWidth >= 1024 })">
                    <h5 id="accordion-collapse-heading-{{ $post_id }}">
                    <button @click="isOpen = !isOpen" type="button" class="flex items-center justify-between w-full p-5 font-medium text-left text-gray-500 focus:ring-4 focus:ring-gray-200 dark:focus:ring-gray-800 dark:border-gray-700 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-gray-800">
                        <span class="text-black-primary text-sm-md-font font-reg420">Opening Hours</span>
                        <!-- SVG when open -->
                    ...
                    <div class="p-5 dark:bg-gray-900">
                        <div class="bg-grey-background rounded-sm-8 p-4 flex items-center justify-between"><span class="text-base-font font-medium text-black-full">Monday - Friday</span><span class="text-grey-font text-base-font font-regular">{{ $fields['mon_fri_opening_hours'] }}</span></div>
                        <div class="bg-white rounded-sm-8 p-4 flex items-center justify-between"><span class="text-base-font font-medium text-black-full">Saturday</span> <span class="text-grey-font text-base-font font-regular">{{ $fields['sat_opening_hours'] }}</span></div>
                        <div class="bg-grey-background rounded-sm-8 p-4 flex items-center justify-between"><span class="text-base-font font-medium text-black-full">Sunday</span><span x-bind:class="{'text-white': $fields['sun_opening_hours'] !== 'Closed', 'text-red-500': $fields['sun_opening_hours'] === 'Closed'}" class="text-red-critical text-base-font font-regular">{{ $fields['sun_opening_hours'] }}</span></div>
                    </div>
                    </div>
                </div>
                </div>
                <div class="flex flex-row justify-between max-w-max-691">
                    <a target="_blank" class="lg:w-[340px] lg:h-[56px] flex justify-center items-center rounded-btn-72 bg-black-full hover:bg-yellow-primary flex items-center border-black border-solid border-3" href="{{ $fields['get_directions_link'] }}">
                        <span class="text-base-font text-white hover:text-black font-medium">Get Directions</span>
                        <svg xmlns="http://www.w3.org/2000/svg" width="33" height="32" viewBox="0 0 33 32" fill="none">
                        <path d="M13.5837 15.668H13.2503V16.0013V19.668H11.2503L11.2503 14.668L11.2503 14.6672C11.2496 14.3749 11.3449 14.1448 11.5354 13.9543C11.7259 13.7638 11.9567 13.668 12.2503 13.668H18.2503H18.5837V13.3346V10.806L22.4456 14.668L18.5837 18.5299V16.0013V15.668H18.2503H13.5837ZM14.6198 28.2993L14.6194 28.2989L3.95269 17.6323C3.71454 17.3941 3.54034 17.1356 3.42506 16.8567L3.11699 16.984L3.42506 16.8567C3.30756 16.5723 3.25033 16.2879 3.25033 16.0013C3.25033 15.7137 3.3076 15.4288 3.42506 15.1446C3.54029 14.8658 3.71437 14.6079 3.9523 14.3707L3.95269 14.3703L14.6194 3.70367C14.8575 3.46551 15.116 3.29131 15.395 3.17604C15.6793 3.05853 15.9637 3.0013 16.2503 3.0013C16.5379 3.0013 16.8228 3.05858 17.107 3.17604L17.2343 2.86797L17.107 3.17604C17.3859 3.29126 17.6438 3.46534 17.8809 3.70328L17.8813 3.70367L28.548 14.3703C28.7861 14.6085 28.9603 14.867 29.0756 15.1459C29.1931 15.4303 29.2503 15.7147 29.2503 16.0013C29.2503 16.2889 29.193 16.5738 29.0756 16.858C28.9604 17.1368 28.7863 17.3947 28.5483 17.6319L28.548 17.6323L17.8813 28.2989C17.6431 28.5371 17.3847 28.7113 17.1057 28.8266C16.8213 28.9441 16.5369 29.0013 16.2503 29.0013C15.9627 29.0013 15.6779 28.944 15.3936 28.8266C15.1148 28.7113 14.8569 28.5373 14.6198 28.2993ZM16.0146 26.9037L16.2503 27.1394L16.486 26.9037L27.1527 16.237L27.3884 16.0013L27.1527 15.7656L16.486 5.09893L16.2503 4.86323L16.0146 5.09893L5.34796 15.7656L5.11225 16.0013L5.34796 16.237L10.6813 21.5703L16.0146 26.9037Z" fill="white" stroke="white" stroke-width="0.666667"/>
                        </svg>
                    </a>
                    <a class="lg:w-[340px] lg:h-[56px] flex justify-center items-center rounded-btn-72 bg-yellow-primary hover:bg-white border-black border-solid border-3 flex items-center" href="{{ $fields['order_for_collection_link']['url'] }}" target="{{ $fields['order_for_collection_link']['target'] }}">
