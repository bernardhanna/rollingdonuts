                    ...
                    @if ($item->children)
                        <div x-cloak x-show.transition.opacity="open" class="submenu flex flex-col absolute left-0 one-xl:left-10 mt-0 bg-white pt-2 z-500 w-[200px]" role="menu" aria-expanded="false">
                            @foreach ($item->children as $child)
                                <a @if($child->target === '_blank') target="_blank" @endif class="{{ !$loop->first ? 'border-t border-black' : '' }} {{ !$loop->last ? 'border-b border-black' : '' }} py-4 px-4 w-full text-reg-font font-reg420 text-black-full {{ $child->classes ?? '' }} hover:bg-yellow-primary hover:text-black-full" href="{{ $child->url }}" role="menuitem">
                                    {{ $child->label }}
                                </a>
                            @endforeach
                        </div>
