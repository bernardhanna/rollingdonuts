@php
$hideOnPages = is_account_page() || is_checkout();
@endphp

<div x-data="{ atBottom: false, hideOnPages: @js($hideOnPages) }" x-init="window.addEventListener('scroll', () => {
...
<div class="fixed bottom-0 left-0 right-0 z-50 pt-4 px-4 pb-1 laptop:hidden bg-black-full border-top-40 transition-opacity duration-500" x-bind:class="{ 'opacity-0': atBottom || hideOnPages, 'opacity-100': !atBottom && !hideOnPages }">
