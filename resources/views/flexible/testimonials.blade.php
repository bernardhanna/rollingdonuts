...
@endphp
<section class="kudos-flexi w-full">
    <svg class="block mobile:hidden relative -bottom-40 z-0" xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 390 367" fill="none">
    ...
    <svg class="hidden mobile:block -bottom-10 md:-bottom-20 laptop:-bottom-40 relative z-0" width="100%" viewBox="0 0 1728 367" fill="none" xmlns="http://www.w3.org/2000/svg">
    ...
    <div class="bg-black-full w-full text-center mx-auto flex justify-center px-4">
    ...
    <div class="w-full bg-black-full z-10 relative">
        <div class="flex flex-col w-full max-w-[1368px] mx-auto px-4">
            @if($kudosTitle)
            <{{ $kudosHeadingTag }} class="text-white text-md-font lg:text-lg-font font-reg420 leading-10">{{ $kudosTitle }}</{{ $kudosHeadingTag }}>
            @endif
            @if($kudosText)
            <div class="text-white font-laca text-sm-font mobile:text-sm-md-font font-light pb-5">{{ $kudosText }}
            </div>
            @endif
        </div>
        <div id="testimonial-slider" class="relative z-50 splide w-full mx-auto flex flex-flow flex-nowrap flex-row justify-start">
            <div class="splide__track w-full max-w-[1530px] ml-auto">
                <div class="splide__list flex justify-start max-w-[1530px]">
                    @if($selectedKudos)
                    ...
                    <div class="splide__slide item p-6 border-3 border-white border-solid rounded-md-32 w-full mobile:w-[497px] mobile:min-w-[497px] h-auto mobile:h-[497px] flex justify-between flex-col">
                        <div class="pb-8">
                            ...
                            <div class="text-white text-base-font mobile:text-reg-font pt-8">
                                {!! get_post_field('post_content', $testimonial->ID) !!}
                            </div>
                        </div>
                        <div class="flex flex-col">
                            <span class="text-xs-font text-white font-laca">{{ get_the_title($testimonial->ID) }}</span>
                            @php
                            $kudosJob = get_field('kudos_job', $testimonial->ID);
                            $kudosImage = get_field('kudos_image', $testimonial->ID);
                            @endphp
                            <span class="text-xs-font mobile:text-xs-font text-white font-laca pb-6">{{ $kudosJob }}</span>
                            @if($kudosImage)
    ...
    <svg class="hidden mobile:block -top-10 laptop:-top-40 relative z-0" xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 1728 367" fill="none">
    ...
    <svg class="block mobile:hidden -top-40 relative z-0" xmlns="http://www.w3.org/2000/svg" width="100%" viewBox="0 0 390 366" fill="none">
