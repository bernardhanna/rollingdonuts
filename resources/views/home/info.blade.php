 ...
 */
?>
<section class="w-full bg-white bg-cover bg-no-repeat bg-center bg-[url('/images/home/white-bg-donuts.png')]">
    <div class="pt-10 px-6">
        <div class="flex flex-col lg:flex-row lg:max-w-max-1514 mx-auto">
            ...
            @if ($eventImage)
                <img class="h-[325px] max-h-max-645 lg:h-full lg:w-full object-cover border-4 border-black-full lg:border-none rounded-20px w-auto mx-auto shadow-md lg:shadow-none" src="{{ $eventImage['url'] }}" alt="{{ $eventImage['alt'] }}">
            @endif
            </div>
            <div class="content lg:w-1/2 lg:flex lg:flex-col lg:justify-center lg:pl-10 xxl:pr-37">
                <h4 class="text-lg-font font-reg420 pt-10 pb-5 leading-3xl">{{ get_field('event_heading') }}</h4>
                <p class="text-reg-font text-black-font pb-20">{{ get_field('event_text') }}</p>
                @php
                $eventButton = get_field('event_button');
                @endphp

                @if ($eventButton)
                    <a class="btn w-full lg:w-inherit text-white text-sm-md-font font-420 bg-black-full border-radius-large py-4 font-reg420 hover:bg-yellow-primary hover:text-black-full" href="{{ $eventButton['url'] }}" class="button">{{ $eventButton['title'] }}<svg class="ml-2 lg:hidden" xmlns="http://www.w3.org/2000/svg" width="34" height="34" viewBox="0 0 34 34" fill="none">
    ...
    <div class="pt-10 lg:pb-24 lg:pt-36 px-6">
        <div class="flex flex-col lg:flex-row-reverse lg:max-w-max-1514 mx-auto">
            <div class="lg:w-1/2">
            @php
            ...
            <div class="content lg:w-1/2 lg:flex lg:flex-col lg:justify-center lg:pl-0 xxl:pr-44">
                <h4 class="text-lg-font font-reg420 pt-10 pb-5 leading-3xl">{{ get_field('giftcard_heading') }}</h4>
                <p class="text-reg-font text-black-font pb-20">{{ get_field('giftcard_text') }}</p>
                @php
                $giftcardButton = get_field('giftcard_button');
                @endphp

                @if ($giftcardButton)
                    <a class="btn w-full lg:w-inherit text-white text-sm-md-font font-420 bg-black-full border-radius-large py-4 font-reg420 hover:bg-yellow-primary hover:text-black-full" href="{{ $giftcardButton['url'] }}" class="button">{{ $giftcardButton['title'] }}</a>
                @endif
            </div>
        </div>
    </div>
